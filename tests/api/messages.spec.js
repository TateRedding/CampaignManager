...
describe("/api/messages", () => {
    ...
    describe("GET /private/:userId", () => {
        it("Returns the data of the user's private messages", async () => {
            const { user, token } = await createFakeUserWithToken({});
            ...
            const response = await request(app)
                .get(`/api/messages/private/${user.id}`)
                .set("Authorization", `Bearer ${token}`);
            expectNotToBeError(response.body);
            expect(response.body.length).toBe(numPrivateMessages);
        });
        ...
        it("Returns a relevant error if logged in user's id is not that of the requested userId", async () => {
            ...
            const response = await request(app)
                ...
                .set("Authorization", `Bearer ${token}`);
            expect(response.status).toBe(403);
            expectToBeError(response.body, "PrivateMessageAccessError");
        ...
        it("Returns a relevant error if no user is logged in", async () => {
            ...
            const response = await request(app).get(`/api/messages/private/${user.id}`)
            expect(response.status).toBe(401);
            expectToBeError(response.body, "UnauthorizedError");
